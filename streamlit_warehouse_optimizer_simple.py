...
def generate_sample_data(num_stores=100):
    ...
    data = []
    for region, bounds in states.items():
...
def optimize_warehouse_locations(data, num_warehouses, sales_weight=0.5):
    ...
    # Initialize KMeans with more iterations for better convergence
    kmeans = KMeans(n_clusters=num_warehouses, random_state=42, n_init=10)
    
    # If sales weight is not 0.5 (balanced), we implement a weighted approach
    if sales_weight != 0.5:
        # Normalize sales
        weights = data['Sales'] / data['Sales'].max()
        
        # Adjust the weight influence (higher value gives more importance to sales)
        weights = weights ** (sales_weight * 2)
        
        # Sample points based on weights for initialization if sales are prioritized
        if sales_weight > 0.5:
            ...
            initial_centers = X[sample_indices]
            kmeans = KMeans(n_clusters=num_warehouses, init=initial_centers, n_init=1)
    
    # Fit the model to find clusters
    cluster_labels = kmeans.fit_predict(X)
    data['Cluster'] = cluster_labels
    
    # Get warehouse locations (centroids)
    warehouse_locations = pd.DataFrame(kmeans.cluster_centers_, columns=['Latitude', 'Longitude'])
